...
feature rankings when the features are correlated.
...
`fuzzyforest` also allows for easy integration with WGCNA via the function 
`wff`. In one function call, WGCNA is used to partition the covariates into 
distinct clusters and fuzzy forests is then applied using this partition.  
